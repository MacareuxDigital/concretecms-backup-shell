...
if [ "$1" = "--all" ] || [ "$1" = "-a" ]; then
    ...
    ZIP_OPTION="${BASE_PATH}/${FILE_NAME}_${NOW_TIME}.sql ${BASE_PATH}/application/ ${BASE_PATH}/packages/"
    NO_OPTION="0"
    ...
    echo "c5 Backup: You've chosen the DEFAULT FILE option. Now we're backing up the SQL and application/."
    ZIP_OPTION="${BASE_PATH}/${FILE_NAME}_${NOW_TIME}.sql ${BASE_PATH}/application/"
    NO_OPTION="0"
    ...
    First Option
    --------------------
    --files OR --file OR -f: back up a SQL and the files in application/. This is default option.
